...
**Статус:** проект (черновик к исполнению другим агентом)
**Целевой репозиторий:** `D:\Documents\Repositories\pi-autoresearch` (v1.9.0, main `ce2ebbc`)
**Донор-репозиторий:** `D:\Documents\Repositories\pi-agile` (main `3ac3449`) — ⚠️ **трогает ДРУГОЙ агент**

> **ГРАНИЦА ОТВЕТСТВЕННОСТИ:** всё, что находится в `D:\Documents\Repositories\pi-agile`
> (включая тестовую инфраструктуру: `tests/typebox-stub.mjs`,
> `tests/typebox-redirect-loader.mjs`, `tests/fake-pi.ts`, `tests/pbt.test.mjs`,
> `tests/smoke.test.mjs`) — **делает другой агент**. Исполнитель этого ТЗ
> работает ТОЛЬКО в pi-autoresearch, читает и КОПИРУЕТ из pi-agile, но НЕ
> модифицирует его. Если по ходу выяснится, что нужна правка в pi-agile
> (например, вынос инфраструктуры в общий пакет) — это отдельная задача,
> описанная в разделе 5, а не действие по этому ТЗ.
...
## 2. Карта кода (факты, проверенные на main `ce2ebbc`)
...
Остальные (8 registerTool + ещё gated): `BestOfN`, `startPhase`, `commitPhase`,
`abortPhase`, `valleyProbe`, `CheckOrthogonal`, `SpaceSearch`, `explore_from`,
`restore_main`, `tree_status`, `compose`, `finalize_research`.
...
- **Побочный эффект на init**: `migrateAutoInstalledHook()` (index.ts:1637 →
  ...
  `~/.pi/agent/extensions/pi-autoresearch.json` (read-only). ⚠️ См. камень №11.

---

## 3. Что переиспользовать из pi-agile

> Все пути — в `D:\Documents\Repositories\pi-agile`. Файлы копируются в
> `tests/` pi-autoresearch. Ничего в pi-agile не менять.
...
| `tests/smoke.test.mjs` | паттерн `test(name, fn)` + `# Results` | сам файл не копировать — харнесс переписать под свои нужды |
...
| `tests/fake-pi.ts` (378 стр.) | скелет fake-pi | `makeCtx`: `sessionId` → `sessionManager.getSessionId()`; выкинуть bdTasks/worktrees/makeFakeUi; оставить active-tools модель, `_lastSystemPrompt`, `execCalls`; `exec` — под git-вызовы авторасёча (rev-parse, update-ref, add, diff --cached --quiet, commit, checkout, cat-file) ИЛИ реальный git в temp-репо (рекомендация — см. 4.4) |
| `tests/fake-pi.ts:83` | `createFakeBridge` | ⚠️ Протокол rpc СОВПАДАЕТ (`subagents:rpc:v1:request/reply:<id>`, проверено: parallel/rpc.ts:23-25) — нужен только для parallel-тулов (BestOfN и т.п.); в PBT v1 можно не включать |
| `tests/pbt.test.mjs:107` | `class RealSystem` | паттерн зеркала рантайма переносится, модель — своя (см. раздел 4) |
...
8. `agentEnd` / `beforeAgentStart` — реальные хуки `agent_end` / `before_agent_start`
   (observer-стиры, реконструкция состояния).
...
процессе и персистит между сценариями (см. камень №10).
...
- `before_agent_start` с agile-подобным systemPrompt — проверка порядка секций
  (если расширение модифицирует systemPrompt — проверить фактом, см. 4.4).
...
- **Git в PBT**: реальный git в temp-репо (рекомендация — git-семантика это
  ...
  проксировать git-команды на реальный git (cwd → temp) ИЛИ заменить на
  `execBashScript`-подобный вызов — проверить, что делает `pi.exec` в prod.
- **`before_agent_start` и systemPrompt**: ПРОВЕРЕНО — расширение дописывает
  `event.systemPrompt + extra` в КОНЕЦ (index.ts:2255): блок «## Autoresearch
  Mode (ACTIVE)» + «## Experiment Protocol (MANDATORY)» + «## Parallel
  Toolkit (CURRENTLY DISABLED)». Добавить smoke-тест на наличие секции
  «Autoresearch Mode (ACTIVE)» в мутированном промпте (fake-pi уже умеет
  записывать `_lastSystemPrompt`). Обратите внимание: в pi-agile роль
  вынесли В НАЧАЛО (до `<project_context>`) — здесь это НЕ сделано, и это
  отдельный вопрос улучшения, НЕ входит в это ТЗ (только зафиксировать
  факт в репорте).
...
## 5. Вынос в отдельный пакет (ОПИСАНИЕ, не действие)

> Это раздел-описание по запросу. Исполнитель по этому ТЗ пакет НЕ создаёт —
> только использует копирование. Создание пакета — отдельная задача (см.
> границу ответственности вверху).

### 5.1 Что и почему выносится
Инфраструктура дублируется: сейчас она лежит в `pi-agile/tests/`, по этому ТЗ
копируется в `pi-autoresearch/tests/`. Третье расширение (pi-slash, pi-rtk-
optimizer и т.д.) получит третью копию. Это кандидат на пакет:
...
### 5.2 Что НЕ выносится
- `RealSystem`-модели и инварианты (специфичны для каждого расширения);
- bd/git-worktree-бриджи, makeFakeUi (специфика pi-agile);
- сценарии/сиды (специфичны).

### 5.3 Нюансы выноса
- **Стратегия:** выносить ТОЛЬКО после того, как инфраструктура стабилизировалась
  в 2+ репозиториях (сейчас — 2-й раз; выносить можно сразу после этого ТЗ).
...
| **4. Smoke real-extension** | lifecycle e2e + порядок systemPrompt (если есть) | smoke зелёный |
| **5. Репорт** | что найдено PBT (реальные баги — с фиксами ТОЛЬКО с тестами); решение по git-стратегии; решение по побочным эффектам на home; статус открытых вопросов 4.4 | репорт в корне репо |
