...
**Статус:** Draft v0.3 (решения D1–D19 согласованы)
**Дата:** 2026-07-23
**Связанные документы:** [PRD.md](./PRD.md)
**Репозиторий:** это и есть [fan92rus/pi-autoresearch](https://github.com/fan92rus/pi-autoresearch) (форк [davebcn87/pi-autoresearch](https://github.com/davebcn87/pi-autoresearch) с Windows-fixes + патч `ensureGatedToolsActive`); parallel-режимы — фича на ветке `feat/parallel-modes`. RPC-поставщик: [pi-subagents](https://github.com/fan92rus/pi-subagents).
...
### 3.4. Phases и valley exploration (допущение временных ухудшений)

Жадный `edit→measure→keep/revert` убивает многошаговые оптимизации: любой путь «ухудшись, потом улучшись» обрывается на первом шаге вниз. Phases решают это транзакцией — внутри фазы авто-revert отключён, валидируется только финальная метрика. Подробно — в §11.5; здесь только контур API.

```ts
// новый статус в log_experiment
status: "explore"   // внутри фазы: NO commit, NO revert; копит правки в working tree

// новые tools
startPhase({ name, rationale, maxSteps=5, maxRegressionPct=25, hardFloorPct=40, budgetSeconds=1800 })
  → { phaseId, phaseBase: <sha> }   // запоминает HEAD=H0, включает explore-режим

commitPhase({ phaseId })
  → { finalMetric, baselineMetric, decision:"keep"|"abort", steps }  // measure full; commit всей цепочки или revert к phaseBase

abortPhase({ phaseId }) → { decision:"abort" }   // revert к phaseBase (или best-checkpoint)

valleyProbe({ phaseId, from:"phaseBase"|"bestCheckpoint", strategies=3, ... })
  → { /* reuse BestOfN mechanics, см. §11.5 М2 */ }
```

Phases ортогональны режимам A/B/C: кандидат режима A может быть многошаговой фазой (worker делает multi-step внутри worktree); режим C beam использует regression-lookahead в pruning (§11.5 М3).
...
## 11.5. Phases и valley exploration (допущение временных ухудшений)

**Проблема:** жадный цикл `edit→measure→keep/revert` — локальный подъём. Шаг с ухудшением = revert, значит любой путь «ухудшись, потом улучшись» (долина в ландшафте) обрывается на первом шаге вниз. Глобальный оптимум за долиной недостижим.

Сценарии, где критично: архитектурный рефакторинг (разобрать монолит → −% → собрать с новой структурой → +%); смена алгоритма (удалить старое → state сломан → написать новое); instrumentation (временные assertions → −% → найти баг → +%); precompute setup (кэш-инфра → −% → использование → +%).

### М1. Phases / транзакции (основа, линейный режим)

Агент декларирует фазу — блок из N правок, внутри которого авто-revert отключён. Валидируется только финальная метрика.

```
startPhase(name="refactor-to-modules", maxSteps=5, hardFloorPct=40)
  edit1 → measure (−12%) → log "explore" (NO revert, NO commit)
  edit2 → measure (−18%) → log "explore"
  edit3 → measure (+8% vs baseline!) → commitPhase → git commit всей цепочки
```

Git mechanics чистые: внутри фазы working tree накапливает правки **без commit/revert**. `startPhase` запоминает `HEAD = H0`. `commitPhase`: если final лучше H0 → `git add+commit` (всё накопленное разом); если хуже → `git checkout -- .` (откат к H0). Буквальная транзакция. Решает 80% случаев, минимально инвазивно — меняем политику keep/revert на «терпеть N шагов».

### М2. Valley probes (продвинутая, переиспользует parallel-инфру)

Фаза дошла до step-limit без улучшения — не сдаваться, а пустить probe: параллельные worktree от `phaseBase` или лучшего промежуточного checkpoint, с разными стратегиями продолжения.

```
фаза зашла в тупик на step 5 (−20%), но best-checkpoint был на step 2 (−5%)
→ valleyProbe({ from:"bestCheckpoint", strategies:3 })
   spawn 3 worker'а в worktree от checkpoint, каждый продолжает своим подходом
→ нашёл путь к улучшению → cherry-pick в main
→ нет → discard, revert к phaseBase
```

Технически это режим A (BestOfN) с `from=checkpoint` и особым промптом «продолжи от этого промежуточного state, ищи улучшение». **Нулевая новая механика** — переиспользуем worktree + fan-out.

### М3. Beam с regression-lookahead (для режима C)

SpaceSearch с K=1 = жадный. С K>1 — луч может «спуститься в долину», пока другие исследуют вершины. **Но наивный pruning top-K по текущей metric убьёт valley-луч.** Нужен lookahead: `allowedRegressionSteps` — луч может регрессировать до R шагов прежде чем быть отсечённым; deferred pruning держит K+R лучей. Без этого beam вырождается в жадность.

### Предохранители «не улететь в худшую сторону»

LLM может «верить», что улучшение за углом — нужны жёсткие предохранители.

| Слой | Параметр | Default | Поведение при превышении |
|---|---|---|---|
| Шаги | `maxPhaseSteps` | 5 | force `commitPhase` — measure, decide keep/abort |
| Глубина долины (soft) | `maxRegressionPct` | 25% | steer агенту «фаза глубокая, пора закрывать» |
| Глубина долины (hard) | `hardFloorPct` | 40% | **авто-abort** (tool форсит revert к phaseBase) |
| Время | `phaseBudgetSeconds` | 1800 (30м) | авто-abort по таймауту |

Критичные детали:
- **Hard floor проверяется на каждом explore-шаге** — measure и так бежит, сравниваем с порогом и авто-abort. Гарантия: худший сценарий = «потратили maxSteps и откатились», а не «упали на 80% и держим».
- **best-checkpoint tracking:** внутри фазы запоминаем лучший промежуточный state (commit в temp-branch). При abort можно откатиться к нему, а не к phaseBase — если фаза была полухорошей.
- **Recovery = всегда revert к известной точке** (phaseBase или best-checkpoint). Ухудшенное состояние никогда не остаётся в main.

### Мульти-объективность (guardrail)

«Ухудшение» по одной метрике ради другой (скорость↑, память↑) — это не valley, а плохой metric design. `checks.sh` должен валидировать инварианты (memory < threshold, tests green) — тогда слепой trade-off ловится на checks, не на metric. Фиксируется в skill как обязательный guardrail.
...
### UC-7: Phases — архитектурный рефакторинг через «долину»

**Контекст:** оптимизируем горячий цикл, метрика = секунды (lower). Гипотеза требует разбить монолитный цикл на модули (шаг 1, ухудшение), вставить precompute-кэш (шаг 2, ухудшение), затем использовать новую структуру (шаг 3, улучшение). Жадный режим убил бы на шаге 1.

```
агент: понимает что оптимизация многошаговая → startPhase(name="modular-cache",
            maxSteps=5, maxRegressionPct=25, hardFloorPct=40)
  phaseBase = HEAD, baselineMetric = 10.0с
агент: edit1 (разбор монолита) → measure 11.8с (+18%) → log explore (NO revert)
агент: edit2 (precompute кэш)   → measure 12.5с (+25%) → log explore; soft-steer «долина глубокая»
агент: edit3 (использовать кэш) → measure 7.2с (−28% vs baseline!) → commitPhase
  final 7.2с лучше baseline 10.0с → git commit всей цепочки edit1..edit3
агент: log_experiment(keep, metric=7.2, "modular+cache phase: -28%")
```

**Hard floor в действии:** если edit3 дал бы 14.5с (+45% > 40% floor) — tool авто-abort: revert к phaseBase, логирует phase-abort, steer «фаза ушла за floor, откат». Худший сценарий = потеря 3 шагов, не ухудшенный main.

**Valley probe:** если фаза дошла до maxSteps=5 без улучшения (застряла на −15%), агент зовёт `valleyProbe({from:"bestCheckpoint", strategies:3})` — 3 параллельных worktree от лучшего checkpoint, каждый со своей стратегией. Один находит выход → cherry-pick в main.
...
| `LogParams.status` enum | +`"budget_exceeded"`, +`"explore"` (внутри фазы: no commit/revert) |
...
| **Новое:** tools `startPhase`/`commitPhase`/`abortPhase` | registerTool; транзакции с regression allowance (§11.5 М1) |
| **Новое:** tool `valleyProbe` | registerTool; переиспользует BestOfN mechanics (§11.5 М2) |
...
- Phases (§11.5): оптимизация многошаговая (refactor, algo swap, instrumentation) → `startPhase`/`commitPhase`; фаза буксует → `valleyProbe`.
- Guardrail: `checks.sh` валидирует инварианты (memory, tests) — ловит слепые multi-objective trade-off'ы.
...
└── parallel/              # NEW
    ...
    └── phases/            # NEW: active phase state (phase_base sha, best-checkpoint, step log)
```
...
| **5. Phases** | `startPhase`/`commitPhase`/`abortPhase` + explore-статус + hard-floor + best-checkpoint | UC-7: 3-шаговый refactor (−% затем +%) проходит; hard floor абортит runaway |
| **6. Режим B** | `CheckOrthogonal` + independence-check + stacking | UC-2: 3 ортогональных патча, Σ улучшение; не-ортогональные → отказ |
| **7. Режим C** | `SpaceSearch` + beam.json + cherry-pick chain + regression-lookahead pruning | UC-3: мультимодальная синтетика, находит глобальный оптимум чаще жадного |
| **8. Valley probes** | `valleyProbe` reuse BestOfN from checkpoint | UC-7 escape: фаза в тупике → probe находит путь |
| **9. Полировка** | CPU-калибровка, advisory, дашборд для раундов | UC-5: auto-lower concurrency при CPU 100% |

MVP = этапы 1–4 (бюджет + режим A). Phases (5) — сразу после MVP, т.к. решает многошаговые оптимизации в линейном режиме. B/C/valley — после.
...
| **D15** | Линейный режим получает **phases** (М1) — транзакции с regression allowance + hard floor | остаться жадным | **Phases** — без них рефакторинг/смена алгоритма невозможны в принципе |
| **D16** | **Hard floor (40%) enforced** на каждом explore-шаге — авто-abort, не advisory | advisory-only | **Enforced** — главный предохранитель «не улететь» |
| **D17** | **Valley probes (М2)** — переиспользуют BestOfN-инфру; добавить в той же стадии, что B/C | только phases | **Да** — даром получаем escape из тупика |
| **D18** | Режим C (beam) — pruning policy с `allowedRegressionSteps` | наивный top-K | **Lookahead** — иначе beam вырождается в жадность |
| **D19** | `checks.sh` как guardrail мульти-объективности (memory/tests) — зафиксировать в skill | игнор | **Зафиксировать** — ловит слепые trade-off'ы |
...
*Конец ТЗ v0.3. Решения D1–D19 согласованы. Архитектура: tool-encapsulated RPC fan-out + phases/valley. Ожидают валидации открытые вопросы §16.*
